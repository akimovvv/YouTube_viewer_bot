...
YOUTUBE_API_KEY = 'AIzaSyDof3WS9hEhUulxtyEFHjaJv8Ram8X7L20'


class Proces_working_app(App):
    ...
    def build(self):
        ...
        self.bt = Button(text='Начать процесс', on_release=self.start_proccess)
        self.bl.add_widget(self.bt)
        al.add_widget(self.bl)
        return al

    def start_proccess(self, instance):
...
class Start_viewer_bot_app(App):
    ...
    def start_process(self, instance):
        if str(self.mainbutton.text[6:]).isdigit():
            ...
            global proces_working_app
            proces_working_app = Proces_working_app(self.mainbutton.text[6:])
            proces_working_app.run()
        else:
